#!/usr/bin/env python3
"""RetailCast India - 28-day forecast pipeline (d_1914..d_1941).
...
Design (see DECISIONS.md):
  ...
  - Final = 0.7*baseline + 0.3*HistGradientBoosting, EXCEPT series with a detected
    recent regime break, which use the baseline alone. Rationale: backtest showed the
    GBM reverts broken series toward their pre-break level (KA_3 RMSSE 0.669 GBM vs
    0.610 baseline), undoing the main data correction.
"""
import argparse, os
...
def prep(sales, cal, prices):
    ...
    pk = prices.set_index(["store_id", "item_id", "wm_yr_wk"])["sell_price"]
    PR = np.full((len(sales), len(alld)), np.nan)
    for i in range(len(sales)):
        for j, w in enumerate(wk):
            try: PR[i, j] = pk.loc[(store[i], item[i], w)]
            except KeyError: pass
    PR = pd.DataFrame(PR).ffill(axis=1).bfill(axis=1).to_numpy()
...
def main():
    ...
    ap.add_argument("--data", default="data"); ap.add_argument("--out", default="submission.csv")
    ap.add_argument("--w-base", type=float, default=0.7)
    a = ap.parse_args()
    print(f"estimator: {'lightgbm' if HAVE_LGB else 'sklearn HistGradientBoosting (fallback)'}")
    ...
    np.save("_baseline.npy", Pb); np.save("_gbm.npy", Pg)
